...
scimgateway.deleteUser = async (baseEntity, id, ctx) => {
  ...
  if (res.length !== 1)
    throw new Error(`${action} error: failed for user id=${id} - not found`);
  const userObj = res[0];
...
scimgateway.modifyUser = async (baseEntity, id, attrObj, ctx) => {
  ...
  const res = users.find({ id: id });
  if (res.length === 0)
    scimgateway.formatError(
      action,
      new Error(`user id=${id} - user not found`)
    );
  if (res.length > 1)
    throw new Error(
  ...
  for (const key in attrObj) {
    if (Array.isArray(attrObj[key])) {
      ...
      const addArr = attrObj[key].filter(
        (el) => !el.operation || el.operation !== "delete"
      );
      ...
      addArr.forEach((el) => {
        if (Object.prototype.hasOwnProperty.call(el, "primary")) {
          if (
            el.primary === true ||
            (typeof el.primary === "string" &&
              el.primary.toLowerCase() === "true")
          ) {
            const index = userObj[key].findIndex(
              (e) => e.primary === el.primary
            );
            if (index >= 0) {
      ...
      for (const el in attrObj[key]) {
        ...
        if (
          ...
          if (attrObj[key][el].primary) {
            ...
            if (
              primVal === true ||
              (typeof primVal === "string" && primVal.toLowerCase() === "true")
            ) {
              const index = userObj[key].findIndex(
                (e) => e.primary === primVal
              );
...
scimgateway.deleteGroup = async (baseEntity, id, ctx) => {
  ...
  if (res.length !== 1) throw new Error(`${action} error: failed for id=${id} - not found`);
...
scimgateway.modifyGroup = async (baseEntity, id, attrObj, ctx) => {
  ...
  const res = groups.find({ id: id });
  if (res.length === 0)
    throw new Error(`${action} error: group id=${id} - group not found`);
  if (res.length > 1)
    throw new Error(
